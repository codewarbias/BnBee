...
const RelatedHotels = ({ location, hotId }) => {
    ...
    const [relHot, setRelHot] = useState([])

    useEffect(() => {
        if (hotels.length > 0 && location) {
            const hotelsData = hotels.filter((hot) => hot.location === location && hot._id !== hotId)
            setRelHot(hotelsData)
        }
    ...
    return (
        <div className='flex flex-col items-center gap-4 my-16 text-[#262626]'>
            ...
            <p className='sm:w-1/3 text-center text-sm'>Simply browse through our extensive list of trusted Hotels.</p>
            <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {relHot.map((item, index) => (
